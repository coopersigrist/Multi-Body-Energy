...
class QuantumDeepField(nn.Module):
    ...
    def forward(self, data, train=False, target=None, predict=False, MO=False):
        ...
        # Return Molecular orbitals (for use as regularizer)
        if MO:
            with torch.no_grad():
                molecular_orbitals = self.LCAO(inputs)
                return idx, molecular_orbitals
...
class MyDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        out = np.load(self.files[idx], allow_pickle=True)
        return out
...
if __name__ == "__main__":
    ...
    file_model = '../output/model--' + setting +'.pt'
